...
impl<Inner: XmlWrapper> XmlChildDefault<XmlList<Inner>> for DynamicChild<'_, XmlList<Inner>> {
    fn ensure(&self) {
        ensure(self.element, self.name());
    }
}

impl<Inner: XmlWrapper> XmlChildDefault<XmlList<Inner>> for Child<'_, XmlList<Inner>> {
    fn ensure(&self) {
        ensure(self.element, self.name);
    }
}
...
fn get<T: XmlWrapper>(element: &XmlElement, name: &str) -> T {
