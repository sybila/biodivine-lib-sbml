...
use crate::layout::{CompartmentGlyph, GeneralGlyph, GraphicalObject, Layout, ReactionGlyph, ReferenceGlyph, SpeciesGlyph, SpeciesReferenceGlyph, TextGlyph};
use crate::SbmlIssue;
use crate::xml::{OptionalProperty, OptionalXmlChild, OptionalXmlProperty, RequiredXmlChild, RequiredXmlProperty, XmlElement, XmlProperty, XmlSubtype, XmlWrapper};
...
impl SbmlValidable for Layout {
    fn validate(&self, issues: &mut Vec<SbmlIssue>, identifiers: &mut HashSet<SId>, meta_ids: &mut HashSet<MetaId>) {
        ...
        self.dimensions().get().validate(issues, identifiers, meta_ids);
...
impl SbmlValidable for CompartmentGlyph {
    fn validate(&self, issues: &mut Vec<SbmlIssue>, identifiers: &mut HashSet<SId>, meta_ids: &mut HashSet<MetaId>) {
        ...
        self.bounding_box().get().validate(issues, identifiers, meta_ids);
...
impl SbmlValidable for ReactionGlyph {
    fn validate(&self, issues: &mut Vec<SbmlIssue>, identifiers: &mut HashSet<SId>, meta_ids: &mut HashSet<MetaId>) {
        ...
        if self.reaction().is_set() {
            ...
            if element.is_none(){
                let message = "Attribute [reaction] does not refer to an existing Reaction element!";
                issues.push(SbmlIssue::new_error("20508", self.xml_element(), message));
            }
        ...
        validate_list_of_objects(&self.species_reference_glyphs().get(), issues, identifiers, meta_ids);         
    }
}
...
impl SbmlValidable for SpeciesReferenceGlyph {
    fn validate(&self, issues: &mut Vec<SbmlIssue>, identifiers: &mut HashSet<SId>, meta_ids: &mut HashSet<MetaId>) {
        ...
        if self.find_by_sid::<SpeciesGlyph>(&species_glyph.get()).is_none() {
            let message = "Attribute [speciesGlyph] does not refer to an existing SpeciesGlyph element!";
            issues.push(SbmlIssue::new_error("20508", self.xml_element(), message));
        }
    }
}
...
impl SbmlValidable for GeneralGlyph {
    fn validate(&self, issues: &mut Vec<SbmlIssue>, identifiers: &mut HashSet<SId>, meta_ids: &mut HashSet<MetaId>) {
        ...
        if self.meta_id_ref().is_set() && self.reference().is_set() {
            apply_rule_20809(self, metaid_ref.get().unwrap(), reference.get().unwrap(),issues);
        }
...
impl SbmlValidable for ReferenceGlyph {
    fn validate(&self, issues: &mut Vec<SbmlIssue>, identifiers: &mut HashSet<SId>, meta_ids: &mut HashSet<MetaId>) {
        ...
        if self.find_by_sid::<GraphicalObject>(&glyph.get()).is_none() {
            let message = "Attribute [glyph] does not refer to an existing GraphicalObject element!";
            issues.push(SbmlIssue::new_error("20508", self.xml_element(), message));
...
impl SbmlValidable for TextGlyph {
    fn validate(&self, issues: &mut Vec<SbmlIssue>, identifiers: &mut HashSet<SId>, meta_ids: &mut HashSet<MetaId>) {
        ...
        if self.meta_id_ref().is_set() && self.origin_of_text().is_set() {
            apply_rule_20809(self, metaid_ref.get().unwrap(), origin_of_text.get().unwrap(),issues);
        }
    }
}
...
pub fn apply_rule_20808<T: SBase>(
    id: Option<SId>,
    element: &T,
    issues: &mut Vec<SbmlIssue>,
){
    if let Some(id) = id{
        match element.find_element_by_sid(&id) { 
            Some(_) => (),
            None => {
                let message = "Attribute containing SId reference does not refer to an existing element!";
                issues.push(SbmlIssue::new_error("20808", element.xml_element(), message));
            }
        }
    }
}
...
pub fn apply_rule_20509<T: SBase>(
    ...
    let message = format!("Attribute [metaidRef] and [{0}] does not refer to the same element!", xml_element.full_name());
...
pub fn apply_rule_20809<T: SBase>(
    ...
    if found_mid.is_some() && found_sid.is_some() && found_mid != found_sid {
        ...
        issues.push(SbmlIssue::new_error("20809", element.xml_element(), message))
    }
}
